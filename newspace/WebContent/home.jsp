...
<!-- 	                                <div class="col-md-offset-1 text-left">
                                        <div class="banner-text text-center m-top-20 banner-flex">
                                            <img src="assets/images/2.jpg" alt="" />
                                        </div>
	                                </div> -->
                                    <div class="col-md-offset-5 text-right">
                                        <div class="banner-text text-center m-top-20 banner-flex">
                                            <h1 class="text-black">${newsTopView.news_title }</h1>
                                            <p class="text-black">${newsTopView.news_summary }</p>
                                            <p class="writen">
                                            	<span class="author m-top-25">
				                                    Writen by ${newsTopView.news_writer} /
	                                                <a href="">${newsTopView.news_media} · ${newsTopView.news_time}</a>         
				                                </span>
                                            </p>
                                            <a class="btn btn-default btn-round btn-size" href="ContentsService.do?news_no=${newsTopView.news_num}" target="_blank" >자세히 보기 </a>
                                        </div>
            ...
            <section class="blog-post-top font-font">
                <div class="container">
                    <div class="row">
                            ...
                            <ul class="blog-post-menu fix">
                                 <c:choose>
                                	<c:when test="${empty param.cate}">
                                		<li id= "alls" class="active_meta"><a href="NewsService.do?num=1">전체</a></li>
                                		<c:choose>
                                			<c:when test="${param.cate != 7}">
                                				<li id = "true"><a href="NewsMetaService.do?num=1&meta=0" class = "fact li" id="tip" title="육하원칙 위주의 사실 전달형 기사 ">스트레이트 기사</a></li>
                                				<li id = "feature"><a href="NewsMetaService.do?num=1&meta=1" class = "discrip li" id="tip" title="사건,인물에 대한 심층취재 기사">피쳐 기사</a></li>
                                			</c:when>
                                		</c:choose>
                                	</c:when>
                                	<c:otherwise>
                                		<li id= "alls" class="active_meta"><a href="NewsService.do?num=1&cate=${param.cate}">전체</a></li>
										<c:choose>
											<c:when test="${param.cate != 7}">
												<li id = "true"><a href="NewsMetaService.do?num=1&cate=${param.cate}&meta=0" class = "fact li" id="tip" title="육하원칙 위주의 사실 전달형 기사 ">스트레이트 기사</a></li>
                                				<li id = "feature"><a href="NewsMetaService.do?num=1&cate=${param.cate}&meta=1" class = "discrip li" id="tip" title="사건,인물에 대한 심층취재 기사">피쳐 기사</a></li>
											</c:when>
										</c:choose>
                                	</c:otherwise>
                                </c:choose> 
                        ...
                        <div class="col-xs-12">
                            <!-- Blog-post start -->
                            <c:forEach var="news" items="${newsList}" begin="0"
								end="${fn:length(newsList)-1}" step="1" varStatus="status">              
	                                <c:choose>
	                                	<c:when test="${empty news.news_img}" >
		                                	 <div >
		                                		<div class="row">
	            	                            	<div class="col-lg-12 p-top-60 p-bottom-60">
	            	                            		<article>
				                                            ...
				                                            <p class="details">
				                                                <a href="ContentsService.do?news_no=${news.news_num}" class="more">
				                                                    자세히 보기
				                                                    <span class="fa fa-angle-double-right"></span>
				                                                </a>
				                                            </p>
				                                            <p class="author m-top-25">
				                                                Writen by ${news.news_writer} /
				                                                <a href="">${news.news_media} · ${news.news_time}</a>
				                                            </p>
				                                        </article>
				                                    </div>
				                                </div>
		                                		...
		                                		<div class="row">
					                                <div class="col-lg-5 p-top-60">
				                                        <div class="blog-post-img">
				                                            <img class = "news_imgs" src="assets/images/${news.news_img}" alt="Post Image" />
				                                        </div>
				                                    </div>
        ...
        <script>
			...
			var totalData = ${sessionScope.newsCount};
    	    var dataPerPage = 5; 
    	    var pageCount = 5;
    	    ...
    	    function paging(totalData, dataPerPage, pageCount, currentPage){
    	        ...
    	        $("#paging a").click(function(){
					...
					if(getParameters("meta") == null && getParameters("cate") == null){
	    	            var temp_url = 'NewsService.do?num='+selectedPage;
					}else if(getParameters("meta") == null && getParameters("cate") != null){
	    	            var temp_url = 'NewsService.do?num='+selectedPage+'&cate='+getParameters("cate");
					}else if(getParameters("meta") != null && getParameters("cate") == null){
	    	            var temp_url = 'NewsMetaService.do?num='+selectedPage+"&meta="+getParameters("meta");
					}else if(getParameters("meta") != null && getParameters("cate") != null){
	    	            var temp_url = 'NewsMetaService.do?num='+selectedPage+"&cate="+getParameters("cate")+"&meta="+getParameters("meta");
					}
